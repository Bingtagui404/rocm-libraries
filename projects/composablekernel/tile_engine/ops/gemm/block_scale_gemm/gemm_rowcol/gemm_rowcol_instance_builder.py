...
# RowCol quant only supports compv3 pipeline with intrawave scheduler
ROWCOL_TRAIT_UNSUPPORTED_COMBINATIONS = {
...
def is_rowcol_trait_combination_valid(pipeline, epilogue, scheduler):
    """Check if a RowCol trait combination is valid.

    Rules:
    - Only compv3 pipeline with intrawave scheduler is supported
    """
    if (pipeline, epilogue, scheduler) in ROWCOL_TRAIT_UNSUPPORTED_COMBINATIONS:
        return False

    return True


class GemmRowColKernelBuilder(GemmKernelBuilder):
    def __init__(
        self,
        kernel_name_prefix,
        working_path,
    ...
    def _validate_tile_config(
        ...
        """Validate tile configuration for RowCol GEMM."""
        a_datatype = self.datatype
        b_datatype = self.datatype
        c_datatype = "fp16" if self.datatype in ["fp8", "bf8"] else self.datatype

        # RowCol-specific: enforce warp_tile_m == warp_tile_n (MFMA requirement)
        if warp_tile_m != warp_tile_n:
            return False

        # RowCol-specific: enforce warp_tile_m <-> warp_tile_k coupling
    ...
    def _generate_trait_combinations(self):
        """Generate valid RowCol trait combinations.
        ...
        for combo in all_combinations:
            ...
            if not is_rowcol_trait_combination_valid(pipeline, epilogue, scheduler):
                logging.debug(
                    f"Skipping unsupported RowCol trait: "
    ...
    def _generate_kernel_instance(self, tile_config, trait_combo):
        """Generate a single RowCol kernel instance header."""
        ...
        # Pipeline mapping - RowCol only uses CompV3 (standard, NOT quant-specific)
        pipeline_impl_map = {
        ...
        instance_code = f"""// Generated RowCol kernel instance for {kernel_name}
#pragma once
...
struct SelectedKernel {{
    ...
    // Quantization group size: RowColQuant uses (1, 1, 1)
    ...
    // Pipeline problem
    using GemmPipelineProblem = ck_tile::GemmRowColTensorQuantPipelineProblem<
        ADataType,
        BDataType,
        AccDataType,
        AccDataType,
...
def _generate_single_kernel_individual(work_item):
    """Worker function to generate a single individual RowCol kernel file."""
    (
        tile_config,
        trait_combo,
        kernel_name_prefix,
    ...
    builder = GemmRowColKernelBuilder(
        kernel_name_prefix, working_path, gpu_target, datatype, layout, config_json
    )
...
def main():
    parser = argparse.ArgumentParser(description="GEMM RowCol kernel instance builder")
    ...
    kernel_name_prefix = "gemm_rowcol"
    builder = GemmRowColKernelBuilder(
        kernel_name_prefix,
        args.working_path,
        args.gpu_target,
    ...
    if args.list_kernels:
        ...
        # Parse trait combo:
        # pipeline_epilogue_scheduler_padM_padN_padK
