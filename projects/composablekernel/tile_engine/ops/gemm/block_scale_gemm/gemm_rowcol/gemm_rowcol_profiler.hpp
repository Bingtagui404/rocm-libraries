...
#include "gemm_rowcol_benchmark.hpp"

class RowColGemmProfiler
{
    public:
    static RowColGemmProfiler& instance(Setting setting)
    {
        static RowColGemmProfiler instance{setting};
        return instance;
    }

    void benchmark(RowColGemmProblem& problem,
                       ...
                       ck_tile::QuantGemmHostArgs&, const ck_tile::stream_config&)>>& callables)
    {
        ...
        // AQ scale tensor: [M, 1] (one float per row)
        // BQ scale tensor: [1, N] (one float per column)
        ck_tile::HostTensor<float> aq_tensor({problem.m_, 1});
        ck_tile::HostTensor<float> bq_tensor({1, problem.n_});
        ...
        ck_tile::QuantGemmHostArgs gemm_args(a_m_k_dev_buf.GetDeviceBuffer(),
                                             ...
                                             problem.n_,
                                             problem.k_,
                                             1, // QK_A = 1 (columns in AQ tensor)
                                             1, // QK_B = 1 (rows in BQ tensor)
                                             problem.stride_a_,
                                             problem.stride_b_,
                                             problem.stride_c_,
                                             0,  // stride_AQ
    ...
    void process_result(const RowColGemmProblem& problem,
                        ...
                        const std::tuple<std::string, float>& kernel_run_result)
    {
        ...
        std::size_t num_byte = sizeof(ADataType) * problem.m_ * problem.k_ +
                               ...
                               sizeof(AQDataType) * problem.m_ + sizeof(BQDataType) * problem.n_ +
        ...
        bool verified_correct =
            !setting_.verify_ ||
            compare_rowcol(name, problem.k_, problem.split_k_, c_m_n_dev_result, c_m_n_host_result);
    ...
    RowColGemmProfiler(const RowColGemmProfiler&)            = delete;
    RowColGemmProfiler& operator=(const RowColGemmProfiler&) = delete;

    private:
    ~RowColGemmProfiler() { kernel_instances_.clear(); }
    RowColGemmProfiler(Setting setting) : setting_(setting) {}
