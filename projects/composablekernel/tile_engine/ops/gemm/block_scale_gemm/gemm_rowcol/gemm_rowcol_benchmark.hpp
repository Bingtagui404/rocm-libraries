...
#include "gemm_rowcol_common.hpp"
...
struct RowColGemmProblem
{
    ...
    friend std::ostream& operator<<(std::ostream& os, const RowColGemmProblem& problem)
    {
        os << "{\n"
...
template <typename ADataType_,
          ...
          typename CDataType_>
auto calculate_rtol_atol_rowcol(const ck_tile::index_t K,
...
/// @brief Compare device and host results for RowCol GEMM
bool compare_rowcol(std::string instanceName,
                    ck_tile::index_t K,
                    ...
                    ck_tile::HostTensor<CDataType>& c_m_n_host_result)
{
    ...
    const auto rtol_atol =
        calculate_rtol_atol_rowcol<ADataType, float, float, BDataType, AccDataType, CDataType>(
...
/// @brief CPU reference implementation for RowCol GEMM
void rowcol_gemm_host_reference(int verify,
                                ...
                                ck_tile::HostTensor<CDataType>& c_m_n_host_result)
{
    if(verify == 1)
    {
        ...
        ck_tile::
            reference_gemm_rowcol_quant<ADataType, float, float, BDataType, AccDataType, CDataType>(
