...
#include "gemm_tensor_benchmark.hpp"

class TensorGemmProfiler
{
    public:
    static TensorGemmProfiler& instance(Setting setting)
    {
        static TensorGemmProfiler instance{setting};
        return instance;
    }

    void benchmark(TensorGemmProblem& problem,
                       ...
                       ck_tile::QuantGemmHostArgs&, const ck_tile::stream_config&)>>& callables)
    {
        ...
        // AQ and BQ scale tensors: [1, 1] (single scalar float each)
        ck_tile::HostTensor<float> aq_tensor({1, 1});
        ck_tile::HostTensor<float> bq_tensor({1, 1});
        ...
        if(setting_.init_method_ == 0)
        {
            ...
            ck_tile::FillUniformDistribution<float>{0.5f, 1.5f}(aq_tensor);
            ck_tile::FillUniformDistribution<float>{0.5f, 1.5f}(bq_tensor);
        }
        else if(setting_.init_method_ == 1)
        {
            ck_tile::FillMonotonicSeq<ADataType>{}(a_m_k);
            ck_tile::FillMonotonicSeq<BDataType>{}(b_k_n);
            ck_tile::FillConstant<float>{1.0f}(aq_tensor);
            ck_tile::FillConstant<float>{1.0f}(bq_tensor);
        }
        ...
        ck_tile::QuantGemmHostArgs gemm_args(a_m_k_dev_buf.GetDeviceBuffer(),
                                             ...
                                             problem.n_,
                                             problem.k_,
                                             1, // QK_A = 1 for tensor quant
                                             1, // QK_B = 1 for tensor quant
                                             problem.stride_a_,
                                             problem.stride_b_,
                                             problem.stride_c_,
                                             0,  // stride_AQ = 0 for tensor quant
                                             0); // stride_BQ = 0 for tensor quant
        ...
        if(setting_.verify_)
        {
            tensor_gemm_host_reference(
                setting_.verify_, a_m_k, b_k_n, aq_tensor, bq_tensor, c_m_n_host_result);
        }
    ...
    void process_result(const TensorGemmProblem& problem,
                        ...
                        const std::tuple<std::string, float>& kernel_run_result)
    {
        ...
        std::size_t num_byte = sizeof(ADataType) * problem.m_ * problem.k_ +
                               sizeof(BDataType) * problem.n_ * problem.k_ + sizeof(float) * 2 +
        ...
        bool verified_correct =
            !setting_.verify_ ||
            compare_tensor(name, problem.k_, problem.split_k_, c_m_n_dev_result, c_m_n_host_result);
        ...
        if(!setting_.csv_filename_.empty())
        {
            ...
            else
            {
                ...
                file << get_rocm_version() << "," << ck_tile::get_device_name() << ","
                     ...
                     << prob.dtype_a_ << "," << prob.dtype_b_ << "," << prob.dtype_aq_ << ","
                     << prob.dtype_bq_ << "," << prob.dtype_acc_ << "," << prob.dtype_c_ << ","
                     << prob.layout_a_ << "," << prob.layout_b_ << "," << prob.layout_c_ << ","
                     << kernel_instance.name_ << "," << std::fixed << std::setprecision(4)
                     << perf.latency_ << "," << perf.tflops_ << "," << perf.bandwidth_ << ","
    ...
    TensorGemmProfiler(const TensorGemmProfiler&)            = delete;
    TensorGemmProfiler& operator=(const TensorGemmProfiler&) = delete;

    private:
    ~TensorGemmProfiler() { kernel_instances_.clear(); }
    TensorGemmProfiler(Setting setting) : setting_(setting) {}
