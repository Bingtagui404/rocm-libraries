...
# Tensor quant only supports compv3 pipeline with intrawave scheduler
TENSOR_TRAIT_UNSUPPORTED_COMBINATIONS = {
...
def is_tensor_trait_combination_valid(pipeline, epilogue, scheduler):
    """Check if a Tensor trait combination is valid.

    Rules:
    - Only compv3 pipeline with intrawave scheduler is supported
    """
    if (pipeline, epilogue, scheduler) in TENSOR_TRAIT_UNSUPPORTED_COMBINATIONS:
        return False

    return True


class GemmTensorKernelBuilder(GemmKernelBuilder):
    def __init__(
        self,
        kernel_name_prefix,
        working_path,
    ...
    def _validate_tile_config(
        ...
        """Validate tile configuration for Tensor GEMM."""
        a_datatype = self.datatype
        b_datatype = self.datatype
        c_datatype = "fp16" if self.datatype in ["fp8", "bf8"] else self.datatype

        # Tensor-specific: enforce warp_tile_m == warp_tile_n (MFMA requirement)
        if warp_tile_m != warp_tile_n:
            return False

        # Tensor-specific: enforce warp_tile_m <-> warp_tile_k coupling
    ...
    def _generate_trait_combinations(self):
        """Generate valid Tensor trait combinations.
        ...
        for combo in all_combinations:
            ...
            if not is_tensor_trait_combination_valid(pipeline, epilogue, scheduler):
                logging.debug(
                    f"Skipping unsupported Tensor trait: "
    ...
    def _generate_kernel_instance(self, tile_config, trait_combo):
        """Generate a single Tensor kernel instance header."""
        ...
        # Pipeline mapping - Tensor only uses CompV3
        pipeline_impl_map = {
        ...
        instance_code = f"""// Generated Tensor kernel instance for {kernel_name}
#pragma once
...
struct SelectedKernel {{
    ...
    // Quantization group size: TensorQuant uses (1, 1, 1)
...
def _generate_single_kernel_individual(work_item):
    """Worker function to generate a single individual Tensor kernel file."""
    (
        tile_config,
        trait_combo,
        kernel_name_prefix,
    ...
    builder = GemmTensorKernelBuilder(
        kernel_name_prefix, working_path, gpu_target, datatype, layout, config_json
    )
...
def main():
    parser = argparse.ArgumentParser(description="GEMM Tensor kernel instance builder")
    ...
    kernel_name_prefix = "gemm_tensor"
    builder = GemmTensorKernelBuilder(
        kernel_name_prefix,
        args.working_path,
        args.gpu_target,
