...
#include "gemm_tensor_common.hpp"
...
struct TensorGemmProblem
{
    ...
    friend std::ostream& operator<<(std::ostream& os, const TensorGemmProblem& problem)
    {
        os << "{\n"
...
template <typename ADataType_,
          ...
          typename CDataType_>
auto calculate_rtol_atol_tensor(const ck_tile::index_t K,
...
/// @brief Compare device and host results for Tensor GEMM
bool compare_tensor(std::string instanceName,
                    ck_tile::index_t K,
                    ...
                    ck_tile::HostTensor<CDataType>& c_m_n_host_result)
{
    ...
    const auto rtol_atol =
        calculate_rtol_atol_tensor<ADataType, float, float, BDataType, AccDataType, CDataType>(
...
/// @brief CPU reference implementation for Tensor GEMM
void tensor_gemm_host_reference(int verify,
                                ck_tile::HostTensor<ADataType>& a_m_k,
                                ck_tile::HostTensor<BDataType>& b_k_n,
                                ck_tile::HostTensor<float>& aq_tensor,
                                ck_tile::HostTensor<float>& bq_tensor,
                                ck_tile::HostTensor<CDataType>& c_m_n_host_result)
{
    if(verify == 1)
    {
        ...
        ck_tile::
            reference_gemm_tensor_quant<ADataType, float, float, BDataType, AccDataType, CDataType>(
                a_m_k, aq_tensor, bq_tensor, b_k_n, c_m_n_host_result);
    }
}
