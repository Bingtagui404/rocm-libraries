...
class TensorGemmBenchmark:
    ...
    def discover_kernels(self) -> List[Path]:
        """Find all benchmark_gemm_tensor_* executables in the build directory."""
        bin_dir = self.build_dir / "bin"
        if not bin_dir.exists():
            print(f"Error: Binary directory {bin_dir} does not exist")
            return []

        kernels = list(bin_dir.glob("benchmark_gemm_tensor_*"))
        if self.verbose:
            print(f"Found {len(kernels)} Tensor kernel executables")
            for k in kernels:
                print(f"  - {k.name}")
        return kernels
    ...
    def extract_kernel_info(self, kernel_path: Path) -> Dict[str, str]:
        ...
        # Parse: benchmark_gemm_tensor_fp8_rcr_compv3_default_intrawave_False_False_True_128x128x128_...
        parts = name.split("_")

        if len(parts) >= 3:
            # Skip "benchmark_gemm_tensor" prefix (3 parts)
            idx = 3
            if idx < len(parts):
...
def main():
    parser = argparse.ArgumentParser(description="Tensor GEMM Kernel Benchmarking Tool")
    parser.add_argument(
    ...
    benchmark = TensorGemmBenchmark(args.build_dir, verbose=args.verbose)

    print("Starting Tensor GEMM kernel benchmark sweep...")
