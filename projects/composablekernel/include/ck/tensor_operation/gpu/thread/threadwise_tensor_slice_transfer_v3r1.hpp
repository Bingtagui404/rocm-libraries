...
struct ThreadwiseTensorSliceTransfer_v3r1
{
    ...
    // Scalar per access sequences for src and dst
                            ...
                            Number<ThreadScratchId> thread_scratch_id = Number<ThreadScratchId>{})
    {
        ...
        constexpr index_t num_access = SrcSpaceFillingCurve::GetNumOfAccess();

        // loop over space-filling curve
        static_for<0, num_access, 1>{}([&](auto iAccess) {
            // Get data index from precomputed lookup table - O(1) instead of O(N²)
            constexpr auto src_data_idx = SrcSpaceFillingCurve::GetIndex(iAccess);
                             ...
                             Number<ThreadScratchId> thread_scratch_id = Number<ThreadScratchId>{})
    {
        ...
        constexpr index_t num_access = DstSpaceFillingCurve::GetNumOfAccess();

        // loop over space-filling curve
        static_for<0, num_access, 1>{}([&](auto iAccess) {
            // Get data index from precomputed lookup table - O(1) instead of O(N²)
            constexpr auto dst_data_idx = DstSpaceFillingCurve::GetIndex(iAccess);
