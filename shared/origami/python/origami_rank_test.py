...
def read_configs_list(library_name):
    ...
    for item in solutions:
        ...
        config.occupancy = max(item['Occupancy'], 1)
        config.workgroup_mapping = item['WGM']
        config.cache_hints_a = item['NTA']
        config.cache_hints_b = item['NTB']
        config.workspace_size = 2**64-1
        config.workspace_size_per_elem_c = 2**64-1
...
def main():
    ...
    for idx, bench_item in enumerate(benchmarks):
        ...
        problem.a_transpose = origami.transpose_t.T if bench_item['transA']=='T' else origami.transpose_t.N
        problem.b_transpose = origami.transpose_t.T if bench_item['transB']=='T' else origami.transpose_t.N
        ...
        rv = origami.rank_configs(problem, hardware, configs_list)
        # results_list = []
        # for rank_item in rv:
        ...
        for kk, (base_item, new_item) in enumerate(zip(base_rankings[idx], rv[:count])):
            try:
                assert base_item['Latency']==round(new_item.latency) and \
                    ...
                    base_item['MI_K']==new_item.config.mi.k, "Solution ranking not identical"
            except AssertionError as e:
                    print('Latency', base_item['Latency'], round(new_item.latency))
                    print('MT_M', base_item['MT_M'], new_item.config.mt.m)
                    print('MT_N', base_item['MT_N'], new_item.config.mt.n)
                    print('MT_K', base_item['MT_K'], new_item.config.mt.k)
                    print('MI_M', base_item['MI_M'], new_item.config.mi.m)
                    print('MI_N', base_item['MI_N'], new_item.config.mi.n)
                    print('MI_K', base_item['MI_K'], new_item.config.mi.k)
                    break
